...
// Import komponen Sidebar dan Footer kamu
    ...
    <nav className="flex items-center justify-between px-6 py-3 bg-black w-full">
      ...
      <div className="flex items-center gap-4 flex-1">
        ...
        <div className="flex items-center bg-[#1f1f1f] rounded-full px-4 py-2 gap-2 w-[360px] hover:bg-[#2a2a2a] transition">
          <svg
            className="w-5 h-5 fill-white opacity-70 shrink-0"
            viewBox="0 0 24 24"
          >
      ...
      <div className="flex items-center gap-6 text-white text-sm font-semibold">
        ...
        <button className="hover:text-white text-gray-300 transition">
          Sign up
        </button>
...
export default function App() {
  return (
    // Container utama: Set tinggi layar penuh (h-screen) dan flex kolom
    <div className="flex flex-col h-screen bg-black overflow-hidden">
      {/* Navbar di bagian atas */}
      <Navbar />

      {/* Area Bawah: Dibagi menjadi Kiri (Sidebar) dan Kanan (Konten Utama) */}
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar di kiri (Atur lebar tetap, misalnya w-64 atau w-[280px]) */}
        <div className="w-[280px] bg-black p-2 flex-shrink-0">
          <Sidebar />
        </div>

        {/* Konten Utama di kanan: Background agak abu-abu khas Spotify */}
        <main className="flex-1 overflow-y-auto bg-[#121212] rounded-lg mr-2 mb-2 p-6">
          {/* Tempat kamu menaruh konten-konten tengah (Hero, Cards, dll) */}
          <div className="min-h-[800px] text-white">
            <h1 className="text-2xl font-bold mb-4">
              Mulai Dengarkan Sekarang
            </h1>
            <p>
              Area ini bisa di-scroll. Taruh komponen kartu musik atau banner di
              sini.
            </p>
            {/* Sisipkan komponen konten lainnya di sini */}
          </div>

          {/* Footer diletakkan di paling bawah area yang bisa di-scroll */}
          <SpotifyFooter />
        </main>
      </div>
    </div>
